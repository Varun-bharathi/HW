...
export function InterviewRoom() {
  ...
  return (
    <div className="h-screen bg-slate-950 flex flex-col overflow-hidden">
          ...
          {streamError ? (
            ...
            <>
              ...
              <div className="absolute bottom-6 right-6 w-48 h-36 rounded-xl overflow-hidden border-2 border-slate-700 bg-slate-900 shadow-2xl">
                ...
                <span className="absolute bottom-1 left-1 text-[10px] font-medium text-white bg-black/50 rounded px-1">You</span>
              </div>
            </>
          )}
        </div>
          ...
          <div className="w-72 border-l border-slate-800 bg-slate-900/80 flex flex-col">
            ...
            <div className="p-3 border-t border-slate-800 flex gap-2">
              <input
                ...
                className="flex-1 bg-slate-800 rounded-lg px-3 py-2 text-sm text-white placeholder:text-slate-500 border border-slate-700 focus:outline-none focus:border-brand-500"
              />
              <button
                onClick={sendMessage}
                className="px-3 py-2 bg-brand-500 hover:bg-brand-600 rounded-lg text-white text-sm font-medium"
              >
                Send
              </button>
            </div>
      ...
      {/* Control bar */}
      <div className="shrink-0 flex items-center justify-center gap-4 py-5 bg-slate-900/80 border-t border-slate-800">
        <button
          onClick={toggleMic}
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${micOn ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-red-500/20 hover:bg-red-500/30 text-red-400'}`}
        ...
        <button
          onClick={toggleCam}
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${camOn ? 'bg-slate-700 hover:bg-slate-600 text-white' : 'bg-red-500/20 hover:bg-red-500/30 text-red-400'}`}
        ...
        <button
          ...
          className={`w-12 h-12 rounded-full flex items-center justify-center transition-colors ${chatOpen ? 'bg-brand-500/30 text-brand-400' : 'bg-slate-700 hover:bg-slate-600 text-white'}`}
          title="Chat"
        >
        ...
        <button
          onClick={endCall}
          className="w-14 h-12 rounded-full bg-red-500 hover:bg-red-600 text-white flex items-center justify-center transition-colors shadow-lg shadow-red-500/30"
          title="End call"
        >
