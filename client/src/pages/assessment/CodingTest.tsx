...
export function CodingTest() {
    ...
    if (submitted && !submitError) {
        return (
            <div className="flex h-[calc(100vh-4rem)] items-center justify-center flex-col gap-4">
                ...
                <h2 className="text-2xl font-bold text-slate-100">Test Submitted!</h2>
    ...
    if (submitted && submitError) {
        return (
            <div className="flex h-[calc(100vh-4rem)] items-center justify-center flex-col gap-4 bg-black">
        ...
        <div className="flex flex-col h-screen lg:h-screen">
            ...
            <header className="h-14 border-b border-slate-800 bg-slate-900 flex items-center justify-between px-6 shrink-0">
                <div className="flex items-center gap-4">
                    <span className="font-semibold text-slate-100">Coding Assessment</span>
                </div>
                <div className="flex items-center gap-6">
                    ...
                    <button
                        ...
                        disabled={submitted}
                        className="px-4 py-1.5 rounded-lg bg-emerald-500 text-slate-100 font-medium hover:bg-emerald-600 disabled:opacity-50"
                    >
                        Submit Test
                    </button>
                </div>
                ...
                <div className="w-1/3 border-r border-slate-800 bg-slate-900 overflow-y-auto flex flex-col">
                    <div className="p-6 flex-1">
                        <div className="flex items-center justify-between mb-4">
                            <h2 className="text-lg font-bold text-slate-100">Question {currentQIndex + 1} of {config.questions.length}</h2>
                            <div className="flex gap-1">
                                {config.questions.map((_, i) => (
                                    <button
                                        key={i}
                                        onClick={() => setCurrentQIndex(i)}
                                        className={`w-6 h-6 rounded flex items-center justify-center text-xs ${currentQIndex === i ? 'bg-brand-500 text-slate-100' : 'bg-slate-800 text-slate-400 hover:bg-slate-700'}`}
                                    >
                                        {i + 1}
                                    </button>
                                ))}
                        ...
                        <div className="prose prose-invert prose-sm max-w-none">
                            {currentQuestion && (
                                <>
                                    <h3 className="text-xl text-slate-100 mb-2">{currentQuestion.content?.split(':')[0]}</h3>
                                    ...
                                    {currentQuestion.examples && currentQuestion.examples.map((ex: any, i: number) => (
                                        <div key={i} className="mb-4">
                                            <p className="font-semibold text-slate-100 mb-1">Example {i + 1}:</p>
                    ...
                    <div className="p-4 border-t border-slate-800 bg-slate-900/50 shrink-0">
                        <div className="flex justify-between">
                            <button
                                ...
                                className="text-sm text-slate-400 hover:text-slate-100 disabled:opacity-30"
                            >
                                Previous
                            </button>
                            <button
